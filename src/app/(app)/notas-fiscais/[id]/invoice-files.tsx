...
export function InvoiceFiles({
  ...
  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const result = await uploadInvoiceFile(invoiceId, {}, formData);
      if (result.error) {
    ...
    <div className="flex flex-col gap-4">
      ...
      {files.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Nenhum arquivo anexado ainda (PDF ou XML da nota, por exemplo).
        </p>
      ) : (
          ...
          {files.map((file) => (
            ...
            >
              ...
              <Button variant="ghost" size="icon" asChild>
                <a href={`/api/notas-fiscais/files/${file.id}`} download>
