...
import { db } from "@/db";
import { contractFiles } from "@/db/schema";
import { getCurrentUser } from "@/lib/auth/current-user";
import { getContractFileBuffer } from "@/lib/storage";
...
export async function GET(
  ...
  let buffer: Buffer;
  try {
    buffer = await getContractFileBuffer(record.contractId, record.storedName);
  } catch {
